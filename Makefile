...
DOCKER_IMAGE_NAME=billbriscoeawg/rpi-busybox-httpd
...
build:
	...
	docker build -t billbriscoeawg/rpi-busybox-httpd .
	docker tag -f $(DOCKER_IMAGE_NAME) $(DOCKER_IMAGE_NAME):latest
...
test:
	docker run --rm billbriscoeawg/rpi-busybox-httpd /bin/busybox echo "Success."
